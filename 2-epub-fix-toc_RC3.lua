...
-- Find entry name inside zip matching a shell pattern
...
-- ============================================================
-- Priority 1: existing NCX labels
-- ============================================================
...
-- ============================================================
-- Priority 3: h1/h2/h3 from xhtml (only used after full extraction)
-- ============================================================
...
-- ============================================================
-- RC3: fast pre-check — read opf and ncx directly from zip
-- Returns: opf_text, ncx_text, opf_entry_name, ncx_entry_name
--          or nil if epub is not patchable
-- ============================================================
...
DocumentRegistry.openDocument = function(self, file, provider)
    ...
    -- RC3: fast pre-check (no temp dir)
    ...
    -- RC3: TOC ok — no temp dir, open immediately
    if #spine == ncx_count then
        logger.info(TAG..": TOC ok — skip (no extraction)")
        return orig_openDocument(self, file, provider)
    end

    -- Only here do we extract to /tmp (mismatch detected)
    ...
    -- ncx_entry is relative path inside zip (e.g. "OEBPS/toc.ncx")
