-- 22-fb2zip-smart RC8: .fb2.zip -> распаковка во /tmp ТОЛЬКО на время сессии (удаляется при закрытии),
-- прогресс/метаданные отвязаны от crengine-пути: перехват DocSettings.open пишет sidecar по пути zip.
-- Место не тратится: fb2 не хранится между сессиями; рядом с zip появляется обычный .sdr (как у всех книг).
...
-- маппинг: временный путь fb2 -> реальный путь zip (пока книга открыта)
local _fb2map = {}
...
-- ============================ перехват DocSettings: прогресс по zip, не по temp fb2 ============================
...
if ok_ds and DocSettings and type(DocSettings.open) == "function" then
    ...
    DocSettings.open = function(a, b, ...)
        -- робастно к вызову точкой DocSettings.open(file) и двоеточием DocSettings:open(file)
...
DocumentRegistry.openDocument = function(self, file, provider)
    if file and file:lower():match("%.fb2%.zip$") then
        ...
        local tempdir = "/tmp/fb2zip_" .. os.time()   -- временный; удаляется при закрытии
        ...
        if fb2_file then
            ...
            _fb2map[fb2_file] = file                  -- прогресс/метаданные пойдут по zip
            ...
            if doc then
                ...
                -- doc.file НЕ трогаем (= fb2_file): crengine читает временный fb2, прогресс идёт по zip через DocSettings-хук
            else
